...
class VendorAccount
{
    ...
    public function withdraw(){
        ...
        $ref = ReferalPayment::where('created_at','<=',$old)->where('vendor_id',$this->vendor->id)->sum('amount') - VendorWithdrawl::where('vendor_id',$this->vendor->id)->sum('amount');
    ...
    public function pending(){
        $currentDate = \Carbon\Carbon::now();
        $old = $currentDate->subDays(7);
        return ReferalPayment::where('created_at','>',$old)->where('vendor_id',$this->vendor->id)->sum('amount') ;
    }
