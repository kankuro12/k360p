...
<main class="main">
	...
	<nav aria-label="breadcrumb" class="breadcrumb-nav">
	 ...
	 @php 
		 $user = \App\model\ShippingDetail::where('user_id',Auth::user()->id)->first();
		 if(!empty($user)){
			 $fname = str_word_count($user->name,1);
		}
	 @endphp 
						...
						<div class="col-lg-9">
							...
							<div class="row">
								<div class="col-sm-6">
									...
									<input type="text" name="fname" class="form-control" @if(!empty($user)) value="{{ $fname[0] }}" @endif required>
								</div><!-- End .col-sm-6 -->

								<div class="col-sm-6">
									<label>Last Name *</label>
									<input type="text" name="lname" class="form-control" @if(!empty($user)) value="{{ $fname[1] }}" @endif required>
								</div><!-- End .col-sm-6 -->
							</div><!-- End .row -->
							...
							<div class="row">
								...
								<div class="col-sm-6">
									<label>Phone *</label>
									<input type="tel" class="form-control" name="phone" @if(!empty($user)) value="{{ $user->phone }}" @endif required>
								</div><!-- End .col-sm-6 -->
							</div><!-- End .row -->

							<label>Email address *</label>
							<input type="email" name="email" class="form-control" @if(!empty($user)) value="{{ $user->email }}" @endif required>

							<label>Order notes (optional)</label>
							<textarea class="form-control" cols="30" rows="4" name="order_message" placeholder="Notes about your order, e.g. special notes for delivery">@if(!empty($user)) {{ $user->order_message }} @endif</textarea>
						</div><!-- End .col-lg-9 -->
