<div>
    <form action="{{ url('vendor/product-attribute/add/' . $productdetail->product_id) }}" method="POST">
        @csrf
            ...
            <div class="col-md-4">
                <ul class="list-group">
                    <li class="list-group-item active">
                        <a href="{{ url('/vendor/product-attribute/del/' . $attribute->id) }}" class="badge">del</a>
                        {{ $attribute->name }}
                    </li>
                    @foreach ($attribute->items as $item)
                        <li class="list-group-item">
                            <a href="{{ url('/vendor/product-attribute-item/del/' . $item->id) }}" class="badge">del</a>
                            {{ $item->name }}
                        </li>

                    @endforeach
                    <li class="list-group-item">
                        <form action="{{ url('vendor/product-attribute-item/add/' . $attribute->id) }}" method="post">
                            @csrf
...
<div id="placement">
    ...
    <hr>
        <form action="{{ url('vendor/product-stock/add/' . $productdetail->product_id) }}" method="POST">
            @csrf
            <div class="row">
            ...
            <table class="table">
                ...
                <tbody>
                    @foreach ( App\model\ProductStock::where('product_id',$productdetail->product_id)->get() as $stock)
                        <tr>
                            <form action="{{url('vendor/product-stock/update/'.$stock->id)}}" method="post">
                                @csrf
                            <td>
